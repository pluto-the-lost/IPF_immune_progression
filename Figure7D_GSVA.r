...
exp<-averages$RNA
es.max_tmp <- gsva(as.matrix(exp), geneset,kcdf="Poisson", verbose=FALSE, parallel.sz=32,min.sz=0, max.sz=1500)
write.table(es.max_tmp,data_file,quote=F,sep='\t')
pheatmap(es.max_tmp,show_rownames=T,show_colnames=T,cluster_cols = F,cluster_rows= T,treeheight_row=30,cellheight=12,fontsize=16,border=FALSE,color = colorRampPalette(c('#4B75A9', "#F6EFBD", '#D23932'))(50),main ='',filename = pdf_file,width = 18,height=26.18,scale='row')
